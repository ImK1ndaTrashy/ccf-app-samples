...
##############
## Add JWT
echo "Proposing use of JWT for auth 1/2: submit set_ca_cert_bundle proposal to network"
"$root_dir"/scripts/submit_proposal.sh --network-url "${server}" \
 --proposal-file "${app_dir}/proposal1.json" \
 --certificate_dir "$certs"

echo "Proposing use of JWT for auth 2/2: submit set_jwt_issuer for microsoft auth"
"$root_dir"/scripts/submit_proposal.sh --network-url "${server}" \
 --proposal-file "${app_dir}/proposal2.json" \
 --certificate_dir "$certs"
